...
import { parseFile, validateQuestions } from './importers.js';
...
async function init(){
  applyTheme();
  document.getElementById('themeBtn').onclick=()=>{localStorage.setItem(THEME_KEY,(document.documentElement.dataset.theme==='dark'?'light':'dark'));applyTheme()};
  document.getElementById('avatarBtn').onclick=()=>showAccountModal();
...
async function go(route, force=false){
  ...
  state.route=route; location.hash=route; await renderRoute(); window.scrollTo(0,0);
}
...
async function renderHome(){
  const supa=db();
  const [q,s,se,a]=await Promise.all([
  ...
  app.innerHTML=`<section class="page">
    <div class="hero"><div><div class="eyebrow">Preparación médica para Perú</div><h1>Entrena. Mide. Mejora.</h1><p class="lead">Banqueo y simulacros para Residentado Médico, ENAM y EsSalud, con progreso, errores, favoritas y revisión de cada pregunta.</p><div class="cta-row"><button class="btn btn-primary" data-go="practice">Empezar a banquear</button><button class="btn" data-go="simulations">Ver simulacros</button></div>${resume}</div>
      <div class="pet-card"><span class="pet-badge">${Math.max(1,Math.floor((state.profile?.xp||0)/250)+1)}</span><div class="pet"><span class="pet-mouth"></span></div><h3>Baqi</h3><p class="muted">Tu compañero de preparación</p><div class="progress"><span style="width:${(state.profile?.xp||0)%250/2.5}%"></span></div><p class="small muted">${state.profile?.xp||0} XP · 🔥 ${state.profile?.streak||0} racha</p></div>
    </div>
    <div class="grid grid-4"><div class="card stats-card"><span class="muted small">Preguntas publicadas</span><strong>${q.count||0}</strong></div><div class="card stats-card"><span class="muted small">Simulacros</span><strong>${s.count||0}</strong></div><div class="card stats-card"><span class="muted small">Sesiones</span><strong>${se.count||0}</strong></div><div class="card stats-card"><span class="muted small">Precisión</span><strong>${pct(correct,attempts.length)}%</strong></div></div>
    <div class="section-head"><div><h2>Elige tu entrenamiento</h2><p class="muted">Todo orientado a exámenes médicos del Perú.</p></div></div>
    <div class="grid grid-4"><div class="card card-click" data-home-go="practice"><div class="bank-icon">🧠</div><h3>Banquear</h3><p class="muted">Filtra por examen, especialidad, tema y subtema. Corrección inmediata o al final.</p></div><div class="card card-click" data-home-go="simulations"><div class="bank-icon">⏱️</div><h3>Simulacros</h3><p class="muted">Exámenes completos cargados por el administrador y revisión posterior.</p></div><div class="card card-click" data-home-go="errors"><div class="bank-icon">🎯</div><h3>Mis errores</h3><p class="muted">Rebanquea las preguntas que fallaste y conviértelas en aciertos.</p></div><div class="card card-click" data-home-go="stats"><div class="bank-icon">📈</div><h3>Progreso</h3><p class="muted">Aciertos, historial y evolución de tu rendimiento.</p></div></div>
  </section>`;
  bindGo();document.querySelectorAll('[data-home-go]').forEach(c=>c.onclick=()=>go(c.dataset.homeGo));
...
async function renderAdminQuestions(){
  const el=document.getElementById('adminContent');el.innerHTML='<div class="card">Cargando...</div>';
  const supa=db();
  ...
  document.getElementById('aqLoad').onclick=loadAdminQuestionsTable;await loadAdminQuestionsTable();
}
...
function renderAdminImport(){
  ...
  el.innerHTML=`<div class="grid"><div class="card"><div class="section-head" style="margin-top:0"><div><h2>Importar preguntas o simulacros</h2><p class="muted">XLSX/CSV BANQO, JSON o PDF de solucionario con la respuesta correcta resaltada en verde.</p></div></div>
  ...
  <div class="dropzone" style="margin-top:16px"><input id="importFile" type="file" accept=".xlsx,.xls,.csv,.json,.pdf" /><p class="muted small">En PDF, BANQO detecta la clave por el resaltado verde y deja todo en PENDIENTE para revisión.</p><button id="parseImport" class="btn btn-primary">Analizar archivo</button></div>
  ...
  document.getElementById('parseImport').onclick=analyseImportFile;
...
async function analyseImportFile(){
  const file=document.getElementById('importFile').files[0];if(!file){notify('Selecciona un archivo');return;}
  const meta=currentImportMeta();if(!meta.bankName||!meta.prefix){notify('Indica nombre y prefijo');return;}
  state.currentImportFile=file;state.importMeta=meta;const box=document.getElementById('importPreview');box.innerHTML='<div class="card">Analizando...</div>';setBusy(true);
  try{
    const questions=await parseFile(file,meta,p=>{box.innerHTML=`<div class="card"><strong>${esc(p.stage)}</strong><p class="muted">Página ${p.page} de ${p.total}</p><div class="progress-line"><span style="width:${pct(p.page,p.total)}%"></span></div></div>`});
    state.importPreview=questions;state.importValidation=validateQuestions(questions);renderImportPreview();
  }catch(e){box.innerHTML=`<div class="card"><h3>Error de importación</h3><p class="danger-text">${esc(e.message||e)}</p></div>`;}finally{setBusy(false)}
}
async function loadSampleImport(kind){
  const path=kind==='bio'?'data/sample_bioetica1.json':'data/sample_simulacro1.json';const data=await (await fetch(path)).json();state.importPreview=data;state.importValidation=validateQuestions(data);state.currentImportFile=null;state.importMeta={bankType:kind==='bio'?'BANCO':'SIMULACRO',examType:'RESIDENTADO',year:null,bankName:kind==='bio'?'Banco Histórico - Bioética 1':'Simulacro 1',prefix:kind==='bio'?'BIO1':'SIM1',duration:null,specialty:kind==='bio'?'Salud Pública y Gestión':'Sin clasificar',topic:kind==='bio'?'Bioética y Deontología':'Sin clasificar',subtopic:kind==='bio'?'Bioética':'Sin clasificar'};renderImportPreview();
}
function renderImportPreview(){
  ...
  <div class="import-status" style="margin:18px 0"><div class="mini-stat"><span class="muted small">Total</span><strong>${s.total}</strong></div><div class="mini-stat"><span class="muted small">Sin error crítico</span><strong>${s.valid}</strong></div><div class="mini-stat"><span class="muted small">Con error</span><strong>${s.errors}</strong></div><div class="mini-stat"><span class="muted small">Advertencias</span><strong>${s.warnings}</strong></div><div class="mini-stat"><span class="muted small">Sin clave</span><strong>${s.missingAnswer}</strong></div><div class="mini-stat"><span class="muted small">Imagen pendiente</span><strong>${s.needsImage}</strong></div></div>
  ...
  <div class="import-preview table-wrap" style="margin-top:14px"><table class="table"><thead><tr><th>#</th><th>ID</th><th>Pregunta</th><th>Clave</th><th>Validación</th></tr></thead><tbody>${v.rows.slice(0,250).map((r,i)=>`<tr class="${r.errors.length?'row-error':r.warnings.length?'row-warning':''}"><td>${i+1}</td><td><span class="code">${esc(r.question.external_id)}</span></td><td>${esc(r.question.stem).slice(0,250)}</td><td><strong>${esc(r.question.correct_answer||'—')}</strong></td><td>${r.errors.length?`<div class="danger-text">${r.errors.map(esc).join('<br>')}</div>`:''}${r.warnings.length?`<div class="warning-text">${r.warnings.map(esc).join('<br>')}</div>`:'<span class="success-text">OK</span>'}</td></tr>`).join('')}</tbody></table></div></div>`;
...
async function commitImport(){
  ...
  try{
    ...
    for(const q of state.importPreview){
      ...
      const copy={...q};delete copy._order;const issues=issueMap.get(q.external_id)||[];copy.review_notes=[copy.review_notes,issues.length?`Validación: ${issues.join(' | ')}`:''].filter(Boolean).join(' ');copy.status='PENDIENTE';copy.created_by=prior?undefined:state.user.id;copy.updated_by=state.user.id;
    ...
    const importItems=state.importPreview.map((q,i)=>{const action=actions.get(q.external_id)||'NUEVA';const issues=[...(issueMap.get(q.external_id)||[]),`Sincronización: ${action}`];return {batch_id:batch.id,row_number:q._order||i+1,external_id:q.external_id,raw_data:{source:state.currentImportFile?.name||'sample'},normalized_data:q,status:action==='SIN_CAMBIOS'?'OMITIDO':'IMPORTADO',errors:issues,linked_question_id:idMap.get(q.external_id)||null};});
    ...
    notify(`Importación: ${sync.new} nuevas · ${sync.modified} modificadas · ${sync.unchanged} sin cambios.`,6000);state.importPreview=null;state.importValidation=null;state.currentImportFile=null;state.adminTab='questions';await renderAdmin();
...
async function renderAdminSimulations(){
  const el=document.getElementById('adminContent');const {data,error}=await db().from('simulation_sets').select('*').order('created_at',{ascending:false});if(error)throw error;
...
async function renderAdminBatches(){
  const el=document.getElementById('adminContent');const {data,error}=await db().from('import_batches').select('*').order('created_at',{ascending:false}).limit(50);if(error)throw error;
